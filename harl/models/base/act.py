    ...
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.multidiscrete_action:
            ...
            for i, action_out in enumerate(self.action_outs):
                ...
                if i == 0:
                    action_distribution = action_out(x, available_actions[:, 0:action_dim])
                else:
                    action_distribution = action_out(x, available_actions[:, action_dim-self.action_dims[i]:action_dim])
                action = (
