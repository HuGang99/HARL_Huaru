...
class AloAgent(Agent):
    ...
    def make_actions(self, actions=None, parse_msg=None, side='red'):
        ...
        for i in range(len(actions)):
            ...
            cur_agent_action = actions[i]   # 拿到当前Agent的网络动作输出
            if th.is_tensor(cur_agent_action):
                cur_agent_action = int(cur_agent_action)
            else:
                cur_agent_action = cur_agent_action

            if cur_agent_action == 14:
                pass
            elif 0<= cur_agent_action < 9:
                # 1. 每个Agent产生移动动作
                # 先拿到上一帧的X，Y，Z坐标
                try:
                    ...
                    last_x, last_y, last_z, last_heading, last_pitch = None, None, None, None, None, None
                # 得到一个新的经纬度坐标（弧度）
                move_new_loc = self.new_loc(last_x, last_y, last_z, last_heading, last_pitch, cur_agent_action)
                cmd_list.append(self.make_move_cmd(move_new_loc, agent_id))
            else:
            # 2. 每个Agent产生攻击动作
                attack_action = cur_agent_action - 9
